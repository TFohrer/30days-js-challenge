(function () {
    ...
    const filteredInventors = inventors.filter((inventor) => {
        return (Math.floor(inventor.year / 100)) === 15;
    });
    ...
    const yearsSum = inventors.reduce((acc, inventor) => {
    ...
    const sortedInventorsLivedYears = inventors.sort((a, b) => {
        return (a.passed - a.year) > (b.passed - b.year) ? -1 : 1;
    });

    console.log('5. Sort the inventors by years lived');
    console.table(sortedInventorsLivedYears);

    // 6. create a list of Boulevards in Paris that contain 'de' anywhere in the name
    // https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris
    function filterDeBoulevards() {
    ...
    // run directly on wikipedia page --> uncommented to prevent error
    ...
    const instances = data.reduce((obj, instance) => {
        obj.hasOwnProperty(instance) ? obj[instance]++ : (obj[instance] = 1);
        return obj;
